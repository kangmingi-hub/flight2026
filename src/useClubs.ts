...
const STORAGE_KEY = 'flight2026_data';
...
function loadClubs(): Club[] {
  try {
    ...
    // Merge with INITIAL_CLUBS to handle new clubs added later
...
export function useClubs() {
  ...
  const updateTarget = useCallback((clubId: string, stat: StatKey, value: number) => {
    setClubs(prev =>
      prev.map(club =>
        club.id === clubId
          ? {
              ...club,
              targets: { ...club.targets, [stat]: value },
              stats: {
                ...club.stats,
                [stat]: { ...club.stats[stat], target: value },
              },
            }
          : club
      )
  ...
  const updateCurrent = useCallback((clubId: string, stat: StatKey, value: number) => {
    setClubs(prev =>
      prev.map(club =>
        club.id === clubId
          ? {
              ...
              stats: {
                ...club.stats,
                [stat]: { ...club.stats[stat], current: value },
              },
            }
          : club
      )
  ...
  const addDailyRecord = useCallback((clubId: string, record: DailyRecord) => {
    setClubs(prev =>
      prev.map(club => {
        ...
        // Recalculate cumulative stats from all records
  ...
  const resetClub = useCallback((clubId: string) => {
    setClubs(prev =>
      prev.map(club =>
        club.id === clubId
          ? {
              ...
              records: [],
              stats: {
                evangelism: { current: 0, target: club.stats.evangelism.target },
                effectiveEvangelism: { current: 0, target: club.stats.effectiveEvangelism.target },
                attendance: { current: 0, target: club.stats.attendance.target },
                baptism: { current: 0, target: club.stats.baptism.target },
              },
            }
          : club
      )
  ...
  const getOverallRate = useCallback(
    (club: Club): number => {
      const stats = Object.values(club.stats) as { current: number; target: number }[];
      const validStats = stats.filter(s => s.target > 0);
      if (validStats.length === 0) return 0;
      const rates = validStats.map(s => Math.min((s.current / s.target) * 100, 100));
      return Math.round(rates.reduce((a, b) => a + b, 0) / rates.length);
    },
    []
  );

  return { clubs, updateTarget, updateCurrent, addDailyRecord, resetClub, getOverallRate };
}
